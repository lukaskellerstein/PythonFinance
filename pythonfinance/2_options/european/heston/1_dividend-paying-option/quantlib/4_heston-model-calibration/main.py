...
def setup_helpers(
    engine, expiration_dates, strikes, data, ref_date, spot, yield_ts, dividend_ts
):
    heston_helpers = []
    grid_data = []
...
def calibration_report(helpers, grid_data, detailed=False):
    ...
    if detailed:
        print(
            "%15s %25s %15s %15s %20s"
            % ("Strikes", "Expiry", "Market Value", "Model Value", "Relative Error (%)")
        )
        print("=" * 100)
...
def setup_model(
    _yield_ts, _dividend_ts, _spot, init_condition=(0.02, 0.2, 0.5, 0.1, 0.01)
):
...
# ------------------------------------------------
# -----------------------------------------------
# CALIBRATION - QuantLib Levenberg-Marquardt Solver
# -----------------------------------------------
# -----------------------------------------------
start = timer()

model1, engine1 = setup_model(
    yield_ts, dividend_ts, spot, init_condition=(0.02, 0.2, 0.5, 0.1, 0.01)
)
...
# QuantLib Levenberg-Marquardt Solver
...
print(
    "theta = %.4f, kappa = %.4f, sigma = %.4f, rho = %.4f, v0 = %.4f"
...
print(f"QL LM1 = {end - start} sec.")

# ------------------------------------------------
# -----------------------------------------------
# CALIBRATION - QuantLib Levenberg-Marquardt Solver (different init_condition)
# -----------------------------------------------
# -----------------------------------------------
start = timer()

model1, engine1 = setup_model(
    yield_ts, dividend_ts, spot, init_condition=(0.07, 0.5, 0.1, 0.1, 0.1)
)
heston_helpers1, grid_data1 = setup_helpers(
    engine1,
    expiration_dates,
...
print(f"QL LM2 = {end - start} sec.")


# ------------------------------------------------
# -----------------------------------------------
# CALIBRATION - SciPy Levenberg-Marquardt Solver
# -----------------------------------------------
# -----------------------------------------------
start = timer()
...
print(f"SciPy LM1 = {end - start} sec.")


# ------------------------------------------------
# -----------------------------------------------
# CALIBRATION - SciPy Levenberg-Marquardt Solver (different init_condition)
# -----------------------------------------------
# -----------------------------------------------
start = timer()
...
print(f"SciPy LM2 = {end - start} sec.")


# ------------------------------------------------
# -----------------------------------------------
# CALIBRATION - Least Squares Method
# -----------------------------------------------
# -----------------------------------------------
start = timer()
...
print(f"SciPy LS1 = {end - start} sec.")


# ------------------------------------------------
# -----------------------------------------------
# CALIBRATION - Least Squares Method (different init_condition)
# -----------------------------------------------
# -----------------------------------------------start = timer()
start = timer()
...
print(f"SciPy LS2 = {end - start} sec.")


# ------------------------------------------------
# -----------------------------------------------
# CALIBRATION - Differential Evolution Method
# -----------------------------------------------
# -----------------------------------------------
start = timer()
...
print(f"SciPy DE1 = {end - start} sec.")


# ------------------------------------------------
# -----------------------------------------------
# CALIBRATION - Differential Evolution Method (different init_condition)
# -----------------------------------------------
# -----------------------------------------------
start = timer()
...
print(f"SciPy DE2 = {end - start} sec.")


# ------------------------------------------------
# -----------------------------------------------
# CALIBRATION - Basin Hopping Algorithm
# -----------------------------------------------
# -----------------------------------------------
start = timer()
...
class MyBounds(object):
    def __init__(self, xmin=[0.0, 0.01, 0.01, -1, 0], xmax=[1, 15, 1, 1, 1.0]):
...
cost_function = cost_function_generator(model5, heston_helpers5, norm=True)
sol = basinhopping(
    cost_function,
    initial_condition,
    niter=5,
...
print(f"SciPy BH1 = {end - start} sec.")


# ------------------------------------------------
# -----------------------------------------------
# CALIBRATION - Basin Hopping Algorithm (different init_condition)
# -----------------------------------------------
# -----------------------------------------------
start = timer()
...
print(f"SciPy BH2 = {end - start} sec.")


# ------------------------------------------------
# -----------------------------------------------
# SUMARRY OF CALIBRATIONS
# -----------------------------------------------
# -----------------------------------------------
df = pd.DataFrame(
    summary,
